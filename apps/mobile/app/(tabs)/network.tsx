  ...
  return (
    ...
    >
      ...
      <View className="flex-row justify-between">
        <Text className="text-muted dark:text-slate-500 text-xs">{t.network.totalReceived} {formatCurrency(item.totalCreditReceived)}</Text>
        <Text className="text-muted dark:text-slate-500 text-xs">{t.network.paid} {formatCurrency(item.totalPaid)}</Text>
      </View>
    </TouchableOpacity>
  );
}
...
function DebtorCard({ item }: { item: DebtorRow }) {
  ...
  return (
    ...
    >
      ...
      <View className="flex-row justify-between">
        <Text className="text-muted dark:text-slate-500 text-xs">{t.network.totalGiven} {formatCurrency(item.totalCreditGiven)}</Text>
        <Text className="text-muted dark:text-slate-500 text-xs">{t.network.received} {formatCurrency(item.totalReceived)}</Text>
      </View>
    </TouchableOpacity>
  );
}
  ...
  return (
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View>
          <Text className="text-text dark:text-slate-100 font-semibold text-sm">{t.externalContacts.screenTitle}</Text>
          <Text className="text-muted dark:text-slate-500 text-xs">{t.externalContacts.noContactsSub.slice(0, 40)}…</Text>
        </View>
      </View>
  ...
  return (
    <View className="flex-1 bg-surface dark:bg-slate-900">
      ...
      {tab === 'suppliers' && supplierList.length > 0 && (
        <View className="mx-4 mb-3 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-900 rounded-xl px-4 py-2">
      ...
      {tab === 'debtors' && debtorList.length > 0 && (
        <View className="mx-4 mb-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-900 rounded-xl px-4 py-2">
