...
export class AddClientToSales1000000000008 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "sale_transactions" ADD COLUMN IF NOT EXISTS "client_name" VARCHAR NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "sale_transactions" ADD COLUMN IF NOT EXISTS "client_phone" VARCHAR NULL`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "sale_transactions" ADD COLUMN IF NOT EXISTS "receipt_id" VARCHAR NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_sale_transactions_receipt_id" ON "sale_transactions" ("receipt_id")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_sale_transactions_client_phone" ON "sale_transactions" ("client_phone") WHERE "client_phone" IS NOT NULL`,
    );
  }
